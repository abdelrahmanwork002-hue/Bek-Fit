...
import { 
  ...
  Loader2, Settings, ListFilter, Trash
} from 'lucide-react';
...
export function AIAgentManagement() {
  ...
  const handleDeleteAgent = async (agent: AIAgent) => {
    ...
    if (error) {
      ...
      toast.error(`Agent '${agent.name}' has been scrubbed from the registry.`);
      ...
      {loading ? (
        <div className="bg-card rounded-3xl border border-border p-20 flex flex-col items-center justify-center text-muted-foreground gap-6 shadow-sm">
           ...
           <div className="text-center">
              <p className="font-black uppercase tracking-[0.3em] text-foreground text-sm">Synchronizing AI Registry</p>
              <p className="text-xs font-bold mt-2 opacity-60">Handshaking with neural network providers...</p>
           </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredAgents.map((agent) => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                ...
                )) || <span className="text-[8px] font-black text-destructive/50 italic uppercase tracking-widest">No Operational Roles</span>}
              </div>
          ...
          {filteredAgents.length === 0 && (
            <div className="col-span-full bg-secondary/20 rounded-3xl border-2 border-dashed border-border p-20 flex flex-col items-center justify-center text-center">
              <Bot className="w-16 h-16 text-muted-foreground/30 mb-6" />
              <p className="text-sm font-black uppercase tracking-widest text-muted-foreground">No autonomous modules detected</p>
              <p className="text-xs font-bold text-muted-foreground/60 mt-2">Adjust filters or initialize a new intelligence module.</p>
            </div>
          )}
        </div>
      )}
